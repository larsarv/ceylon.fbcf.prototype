import eu.arvidson.ceylon.fbcf.component { simpleROValue, Template, Fragment, roroot, Text, Repeat, Value, noargs, stringList, BindingLookup, EventHandlerRegistry, initializeAndAppendToBody, Tuple1 }
...
shared class TodoList() {
	...
	shared variable { TodoItem *} items = empty; // [ for (i in 1..100) TodoItem("Item ``i``") ].sequence;
...
shared class TodoMvc() {
	...
	shared void run() {
		initializeAndAppendToBody((BindingLookup bindingLookup, EventHandlerRegistry eventHandlerRegistry) {
			return buildTemplate().instantiate(bindingLookup, simpleROValue(TodoList()));
		});
	}
}
